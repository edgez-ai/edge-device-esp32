                                            ...
                                            uint16_t peer_independent_ethertype)
{
    ...
    /* Peer-independent topology reports are already complete NetworkPacket
     * protobuf messages. They do not use directed mesh routing or an RA next
     * hop, so send the protobuf directly without the route prefix used by
     * normal routed traffic. */
    if (peer_independent_ethertype == HALOW_SYNC_REPORT_ETHERTYPE) {
        MESH_DEBUG_LOGI(
            "HaLow topology report send peer-independent protobuf len=%u",
            (unsigned)payload_len);
        EdgezRadioError err = s_halow.sendPeerIndependentBeacon(payload, payload_len);
    ...
    uint8_t dest_mac[EDGEZ_ROUTE_MAC_LEN] = {0};
    uint8_t next_hop_mac[EDGEZ_ROUTE_MAC_LEN] = {0};
    /* Keep the final destination in the Ethernet DA/mesh DA while explicitly
     * supplying the route-table next hop as the radio RA. */
    bool directed = u64_to_mac(to, dest_mac);
    bool have_next_hop = u64_to_mac(next_hop, next_hop_mac);

    MESH_DEBUG_LOGI(
             "HaLow NetworkPacket vendor send message_id=%016llx-%016llx seq=%lu from=0x%012llx to=0x%012llx next=0x%012llx dst=%02x:%02x:%02x:%02x:%02x:%02x max_hop=%lu hop=%lu payload=%u route_payload=%u",
             (unsigned long long)message_id_high,
             (unsigned long long)message_id_low,
             (unsigned long)sequence,
             (unsigned long long)(from & 0xffffffffffffULL),
             ...
             directed ? dest_mac[0] : 0xff, directed ? dest_mac[1] : 0xff,
             directed ? dest_mac[2] : 0xff, directed ? dest_mac[3] : 0xff,
             directed ? dest_mac[4] : 0xff, directed ? dest_mac[5] : 0xff,
             (unsigned long)max_hop,
             (unsigned long)hop,
             (unsigned)payload_len,
             (unsigned)off);

    EdgezRadioError err = directed ? s_halow.sendVendorPayloadToVia(
                                   dest_mac,
                                   have_next_hop ? next_hop_mac : dest_mac,
                                   route_payload,
                                   off)
                             : s_halow.sendVendorPayload(route_payload, off);
    return (err == EDGEZ_RADIO_OK) ? ESP_OK : ESP_FAIL;
}
